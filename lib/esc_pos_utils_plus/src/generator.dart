...
class Generator {
  ...
  List<int> row(List<PosColumn> cols, {bool multiLine = true}) {
    ...
    for (int i = 0; i < cols.length; ++i) {
      ...
      if (!cols[i].containsChinese) {
        ...
        // Calculate total encoded length for all lexemes in this column so alignment is correct
        int totalBytesLen = 0;
        for (var j = 0; j < lexemes.length; ++j) {
          totalBytesLen += _encode(
            lexemes[j],
        ...
        for (var j = 0; j < lexemes.length; ++j) {
          bytes += _text(
            ...
            colWidth: cols[i].width,
            isKanji: isLexemeChinese[j],
            totalTextBytes: colIndex != null ? totalBytesLen : null,
          );
  ...
  List<int> _text(
    ...
    if (colInd != null) {
      ...
      if (colWidth != 12) {
        ...
        // use totalTextBytes when provided (for mixed lexemes alignment)
        final int usedBytes = totalTextBytes ?? textBytes.length;
        final double textLen = usedBytes * charWidth;
  ...
  List<int> _mixedKanji(
    ...
    // Compute total encoded length for alignment across lexemes
    int totalBytesLen = 0;
    for (var i = 0; i < lexemes.length; ++i) {
      totalBytesLen += _encode(lexemes[i], isKanji: isLexemeChinese[i]).length;
    }
    ...
    for (var i = 0; i < lexemes.length; ++i) {
      bytes += _text(
        ...
        maxCharsPerLine: maxCharsPerLine,
        totalTextBytes: colInd != null ? totalBytesLen : null,
      );
      // Define the absolute position only once (we print one line only)
      colInd = null;
    }
